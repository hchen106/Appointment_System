...
void timeslot::generateLabel() {
    ...
    for(int hour = open_hour, min = open_min, row = 0, col = 0, i = 0, checkmin = 0; checkmin <= total_min; checkmin = checkmin + gap_time, min = min + gap_time, row++, i++) {
        ...
        if(db.checkBooking(getDate(), hour, min)) {
...
void timeslot::on_book_btn_clicked()
{
     DbManager db(path);
     QString name = ui->name_entry->text();
     QString phone = ui->phone_entry->text();
     QString email = ui->email_entry->text();
     QString booktime = ui->book_time_entry->currentText();
     QStringList list = booktime.split(":");
     if(!db.checkExistClient(name, phone, email)) {
         QMessageBox::warning(this, "add booking", "Client info not found, please add new client first");
     }else if(db.addBooking(name, phone, email, getDate(), list[0], list[1])) {
        QMessageBox::information(this, "add booking", "Appointment is Successfully added");
    }else{
        QMessageBox::warning(this, "add booking", "Appointment is fail to added");
    }
}
